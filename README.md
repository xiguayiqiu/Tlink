...
- **🔧 修复解压权限问题** - 修复了 Linux 系统下接收压缩文件时出现的 `permission denied` 错误，确保临时文件和目录有足够的创建权限
- **📁 优化目录解压** - 修复了目录压缩包解压后文件不在目标文件夹的问题
- **🔒 权限安全处理** - 解压时先用安全权限创建文件，完成后再应用原始权限，避免权限不足导致失败
- **✅ 修复传输完成消息处理** - 修复了传输完成后，继续接收旧消息的问题
- **🐞 优化错误处理** - 光标无法移动的问题
- **🐞 优化错误处理** - 修复Windows无法使用cmd命令的问题！
## 许可证
